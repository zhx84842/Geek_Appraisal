...
答：哈夫曼树的特点是没有度为1的结点，只有度为0或2的点，叶结点度数为0，在非空二叉树中，度数为2的结点数等于度数为0的结点数减1，所以结点总数是2n-1。
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
答：只读写小写字母的话，只需将代码中运用的字符'A'包括"0X41"的全部改为'a';如果同时具有大小写的话，根据ASCII码表，A~Z对应65~90，而a~z对应97~122，记录各个字符的出现次数的数组大小应为2*26+1，遍历所有字母时，因为Z与a之间存在6个其他字符，所以遍历所有字符时，要循环58次，其他过程基本一样。
