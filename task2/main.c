...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]!=0)
        {
            arr_small[count2++] = arr_big[i];	// 将arr_big数组中的非零值同步到数组arr_small中，有多少个数据对应有多少个字母类型   
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*判断该结点是否既没有左结点又没有右结点，即是否为叶结点*/
        {
...
void huffmanTree(int w[],int huffman[][4],int n)
{
    ...
    for(int i = n; i < 2 * n - 1; i++)
    {
        ...
        huffman[i][1] = i1;         //左子结点 
        huffman[i][2] = 12;         //右子结点 
        huffman[i][3] = huffman[i1][3] + huffman[i2][3];             //权重
        huffman[i1][0] = huffman[i2][0] = i;                         //子节点对应的父节点
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*排除尚未通过合并子节点重新赋值的结点*/
        {
            break;
        }
        if(huffman[i][3] < min1 && huffman[i][0] == -1)  /*先判断它是否小于min，再根据它没有父结点来判断它是可以合并的子结点，二者需同时满足*/
        {                                                
            min1= huffman[i][3];                              
            ii[0] = i;
...
{
    int length = 0;
    if(huffman[i][0] == -1)     /*判断是否没有父结点，没有父节点即自身为父节点，即判断该结点本身是不是父结点*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)   //i<count  i++循环 
{
    ...
    {   
        ...
        if (huffman[father][1] == j)	//判断当前结点的父结点左子树是否为当前结点
        {
        ...
        j = father;		//往上朔源，更新当前结点
        father = huffman[j][0];	//同理（当前结点更新后），更新当前结点的父亲结点
